...
// The keystroke search is debounced through useDebouncedSearch (SEARCH_DEBOUNCE_MS): at
// normal typing speed only the last 2-4 keystrokes of a query reach the main process,
// which otherwise blocks presTake/presGoLive behind every intermediate search. The
// import-completed re-search stays un-debounced — it is one deliberate call, not a
// keystroke.
...
export function SongsMode({ keyHandlerRef, active }: SongsModeProps): JSX.Element {
  ...
  // Re-query on every keystroke (debounced, #15) and field switch (immediate). Empty query
  // shows the library instead (displayedRows only reads `results` when the query is
  // non-empty, so no reset needed). In Title mode the subordinate "Also in lyrics" hint is
  // fetched only AFTER the title results land and only when they are thin
  // (< SECONDARY_TITLE_MAX) — the common fat case previously paid a full lyric-scored
  // search per keystroke (~12x the title cost) and threw the result away
  // (secondaryLyricRows returns [] when titles are not thin).
  useDebouncedSearch(q.trim() ? q : null, field, refreshSearch);
