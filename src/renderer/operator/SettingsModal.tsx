...
export function SettingsModal({
  ...
  const installedTagStyle: CSSProperties = {
    fontSize: '12.5px',
    color: T.dim,
    whiteSpace: 'nowrap'
  }
  ...
  const renderMessageInstallStatus = (): JSX.Element => {
    if (
      messageProgress &&
      (messageProgress.phase === 'downloading' || messageProgress.phase === 'installing')
    ) {
      const label =
      ...
      return (
        <>
          ...
          <span style={progressLabelStyle}>
            {p.phase === 'downloading' ? 'Downloading…' : 'Installing…'}
          </span>
        </>
      )
    }
    ...
    if (entry.installed) {
      return (
        <>
          ...
          <button
            style={ghostBtnStyle(confirmId === entry.id)}
