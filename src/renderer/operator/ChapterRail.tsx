...
export function ChapterRail({
  ...
  const rowStyle = (isLive: boolean, isCued: boolean, planned: boolean): CSSProperties => ({
    display: 'block',
    width: '100%',
    textAlign: 'left',
    padding: '11px 13px',
    ...
    <div style={panelStyle}>
      <div style={{ padding: '14px 15px 10px', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: '11px', letterSpacing: '0.1em', color: T.faint, fontWeight: 600 }}>
            {book} {ch}
          </div>
          <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '10px', color: T.scripture }}>{plannedLabel}</div>
        </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '0 12px 14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {verses.map((v) => {
          ...
          return (
            <button key={v} style={rowStyle(isLive, isCued, planned)} onClick={() => onSelect(v)}>
