...
import { usePreState } from './useHelm';
...
export function PreServiceMode({ themeMode, active: _active }: PreServiceModeProps): JSX.Element {
  ...
  const projColor = engaged ? PROJECTING_GREEN : T.faint;
  ...
  const projDotStyle: CSSProperties = {
    ...
    animation: engaged ? 'lecPulse 1.6s ease-in-out infinite' : 'none'
  };
  const projText = engaged ? 'PROJECTING' : 'OFF SCREEN';
      ...
      <div style={preRailStyle}>
        <div style={{ padding: '13px 13px 9px', flexShrink: 0 }}>
          ...
          <div style={{ fontSize: '11.5px', color: T.faint, marginTop: '5px', lineHeight: 1.4 }}>
            Tap a card to show it now. Toggle cards in or out of the rotation.
          </div>
        </div>
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '0 10px 12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {cards.map((card, i) => {
            ...
            const rowStyle: CSSProperties = {
              ...
              borderRadius: '11px',
              cursor: 'pointer',
              opacity: card.enabled || isShowing ? 1 : 0.55,
              background: isShowing ? (dark ? '#221d10' : '#fbf1da') : T.panel2,
              boxShadow: isShowing ? `inset 0 0 0 1.5px ${T.accent}88` : `inset 0 0 0 1px ${T.hairline}`
            };
            const labelStyle: CSSProperties = {
              ...
              fontWeight: 500,
              color: isShowing ? T.accent : T.faint
            };
            const snippetStyle: CSSProperties = {
              fontSize: '12.5px',
              lineHeight: 1.45,
              fontWeight: 500,
              color: isShowing ? T.text : T.dim,
              overflow: 'hidden',
              display: '-webkit-box',
              WebkitLineClamp: 2,
