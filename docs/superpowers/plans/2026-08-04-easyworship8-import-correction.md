...
- **Never issue `WHERE`, `ORDER BY`, `DISTINCT` or `GROUP BY` against a text column** of an EasyWorship table. `title`, `author`, `words`, `copyright`, `tags`, `reference_number` and `filename` are declared `COLLATE UTF8_U_CI`, which `better-sqlite3` cannot register. `COUNT(*)` and anything keyed on `rowid` are safe.
